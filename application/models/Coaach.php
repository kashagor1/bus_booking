...
Class Coaach extends CI_Model{
    ...
    public function update_coach($data){
        ...
        $qq = "UPDATE coach SET route_id='$data[cc_route_id]', coach_type='$data[coach_type]', vehicle_number='$data[vehicle_number]', supervisor_no='$data[supervisor_no]', seat_layout='$data[seat_layout]', departure='$data[departure]', arrival='$data[arrival]', main_boarding='$oname', final_destination='$dname', total_fare='$data[total_fare]' WHERE coach_id='$data[coach_id]'";
     //  echo $qq;die;
        if($this->db->query($qq)){
            return true;
        }else{
